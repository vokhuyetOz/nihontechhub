...
import { ESource } from '@module/crawl/constants/crawl.header';
import { ELanguage } from '@common/enums';

@Entity({ collection: 'news' })
export class News extends BaseMongoEntity {
  @ApiProperty()
  @IsString()
  @IsNotEmpty()
  @IsDefined()
  ...
  getDataByLanguage(code: ELanguage = ELanguage.JA) {
    ...
    let defaultAuthor = {
      ...
      bio: 'A platform that delivers the latest technology news and startup information from Japan and around the world. Connecting innovation and opening the future with knowledge and inspiration.',
    };
    if (code === ELanguage.JA) {
      defaultAuthor = {
        avatar:
          'https://images.unsplash.com/photo-1485827404703-89b55fcc595e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
        name: 'NihonTechHub',
    ...
    const result: Record<string, any> = {
      id: this.id,
      author: this.author ?? defaultAuthor,
      link: this.link,
